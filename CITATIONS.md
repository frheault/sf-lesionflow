...
* **BAWIL Proxy (`SEGMENTATION_BAWIL`)**:
  * Likely named for and inspired by Bashiri Bawil M, Shamsi M, Shakeri Bavil A. *Simultaneous segmentation of ventricles and normal/abnormal white matter hyperintensities in clinical MRI using deep learning* (preprint, 2025) — a pix2pix-based method distinguishing normal periventricular hyperintensities from pathological lesions, matching this proxy's stated purpose. This attribution is a name/topic match, not a confirmed link to the original pipeline author's intent, and the source itself is an unreviewed preprint. Heuristic reproduction only — does not load trained weights.
